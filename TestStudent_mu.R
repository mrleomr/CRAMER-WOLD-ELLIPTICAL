...
save.image("student_mu.Rdata")
base[1,]<-c(0.055,0.065,0.051,100)
names(base)<-c("dim=5","dim=10","dim=50","N")
base$mu<-rep(muses,length(MUESTRAS))
library(reshape2)
base

base2<-melt(base, id=c("mu","N"))
library(tidyverse)
library(showtext)
library(thematic)
...
# Line plot
p <- base2 %>% 
  ggplot(aes(x = mu , y= Power, col = N)) +
  geom_line(size = 0.5) + facet_wrap(~ Dimension, nrow = 1)+
  scale_color_manual(values = colors) +theme_bw()+
  labs(x = ~ paste("Position parameter (", mu[2], ")"), y = 'Power function', col = 'Sample size')

setwd("~/Dropbox/2022_CramerWold_Elliptical/CodeR")
pdf("Power_mu.pdf", height=3, width = 9)
p
dev.off()
